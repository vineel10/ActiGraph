                ...
                if (File.Exists(absoluteFilePath))
                {
                    ...
                    var detailSubjectRecords = (from DataRow dRow in result.Rows
                        ...
                        select new SubjectData()
                        {
                            ...
                            VeryVigorous = 0, //Convert.ToDouble(dRow["Very Vigorous"]),
                            Sedentary = Convert.ToDouble(dRow["Sedentary"]),
                            SedentaryBouts = Convert.ToInt64(dRow["Number of Sedentary Bouts occurring on this day"]),
                            ...
                            Moderate10 =
                                Convert.ToDouble(dRow["Total time of Freedson (1998) bouts occurring on this day"])
                        }).OrderBy(c => c.ID);
                    ...
                    subjectRecordsList.AddRange(from subjectItem in distinctSubjectId
                        ...
                        let subjectDatas =
                            (IList<SubjectData>) (subjectRecords as IList<SubjectData> ?? subjectRecords.ToList())
                        let subjectvalidRecords = GetSubjectValidRecords(subjectDatas)
                        let validSubjectRecords =
                            (SubjectData[]) (subjectvalidRecords as SubjectData[] ?? subjectvalidRecords.ToArray())
                        ...
                        select new SubjectRecords()
                        {
                            ...
                            SubjectValidData = validSubjectRecords,
                            SubjectValidAverages = validSubjectAvgRecords,
